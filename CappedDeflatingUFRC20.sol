...
contract DeflatingUFRC20 is auth, ReentrancyGuard, IUFRC20 {
    ...
    uint  public _totalSupply; uint public immutable _maxSupply; // Max supply cannot be changed after creation.
    ...
    function _mint(address to, uint value) internal {
        if (_totalSupply.add(value) > _maxSupply)
    ...
    function _transfer(address from, address to, uint value) internal nonReentrant returns (bool) {
        uint burnAmount = value / uint256(100).div(taxRate);
    ...
    function approve(address spender, uint value) private override returns (bool) {
