...
contract UFRC20 is auth, ReentrancyGuard, IUFRC20 {
    ...
    uint  internal _totalSupply; uint public immutable _maxSupply; // Max supply cannot be changed. 0 for no MaxSupply
    ...
    constructor(uint maxSupply_, string memory name_, string memory symbol_) {
    _name = name_;
    _symbol = symbol_;
    _maxSupply = maxSupply_*10**18;
        uint chainId;
        assembly {
            chainId := chainid()
        }
  ...
  fallback() external {  // fallback protection
    ...
    function _mint(address to, uint value) internal {
        if (_totalSupply.add(value) > _maxSupply && _maxSupply > 0)
        value = _maxSupply.sub(_totalSupply);
    ...
    function _transfer(address from, address to, uint value) internal nonReentrant {
        _balanceOf[from] = _balanceOf[from].sub(value);
        _balanceOf[to] = _balanceOf[to].add(value);
